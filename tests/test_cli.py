...
def test_cli_both_pcap_and_live(capsys: pytest.CaptureFixture[str]) -> None:
...
@patch('src.cli.sniff')
def test_cli_live_default_interface(mock_sniff: MagicMock, capsys: pytest.CaptureFixture[str]) -> None:
...
@patch('src.cli.sniff')
def test_cli_live_specified_interface(mock_sniff: MagicMock, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_cli_live_keyboard_interrupt(mock_sniff: MagicMock, capsys: pytest.CaptureFixture[str]) -> None:
    with patch.object(sys, 'argv', ['src/cli.py', '--live']):
        main()
